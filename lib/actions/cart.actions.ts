...
export async function addItemToCart(data: CartItem) {
  try {
    ...
    if (!cart) {
      return await createNewCart(item, sessionCartId, userId, product);
    }
    ...
    const isExisting = cart.items.some((x) => x.productId === item.productId);

    return {
      success: true,
      message: `${product.name} ${isExisting ? 'updated in' : 'added to'} cart`,
    };
  } catch (error) {
    return {
      success: false,
